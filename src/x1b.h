...
#include "poly-1b.h"
#include "kit.h"
#include <autodiff/forward/real.hpp>  // Add autodiff header
...
// Template version of var_intra that works with any numeric type
template<typename T>
inline T var_intra_t(const T& d0, const T& k, const T& d)
{
    //return kit::MathFunctions<T>::exp(k*(d0 - d));
    return exp(k*(d0 - d));
}
...
{
    return var_intra_t<double>(d0, k, d);
}
...
// Templated versions for autodiff
template<typename T>
void cart_to_vars_t(const T* xyz, T* vars);

template<typename T>
T value_t(const T* xyz);

// New functions to compute derivatives
